...
**A light-weight and powerful meta-prompting, context engineering and spec-driven development system for Codex CLI, OpenCode, and Gemini CLI.**
...
GSD fixes that. It's the context engineering layer that makes Codex CLI reliable. Describe your idea, let the system extract everything it needs to know, and let Codex CLI get to work.
...
1. **Runtime** — Codex CLI, OpenCode, Gemini, or all
...
```bash
# Codex CLI
npx get-shit-done-codex --claude --global   # Install to ~/.codex/
npx get-shit-done-codex --claude --local    # Install to ./.codex/
...
If you prefer not to use that flag, add this to your project's `.codex/settings.json`:
...
GSD's codebase mapping and analysis commands read files to understand your project. **Protect files containing secrets** by adding them to Codex CLI's deny list:

1. Open Codex CLI settings (`.codex/settings.json` or global)
...
- Restart Codex CLI to reload slash commands
- Verify files exist in `~/.codex/skills/gsd/` (global) or `./.codex/skills/gsd/` (local)
...
OpenCode and Gemini CLI are now natively supported via `npx get-shit-done-codex`.
