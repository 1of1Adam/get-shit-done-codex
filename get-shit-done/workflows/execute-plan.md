...
```bash
INIT=$(node ~/.codex/get-shit-done/bin$gsd-tools.js init execute-phase "${PHASE}" --include state,config)
```
...
**Pattern A:** init_agent_tracking → spawn_agent(agent_name="gsd-executor", model=executor_model) with instructions: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
When spawned via spawn_agent and hitting checkpoint: return structured state (cannot interact with user directly).

**Required return:** 1) Completed Tasks table (hashes + files) 2) Current Checkpoint (what's blocking) 3) Checkpoint Details (user-facing content) 4) Awaiting (what's needed from user)
...
node ~/.codex/get-shit-done/bin$gsd-tools.js state advance-plan

# Recalculate progress bar from disk state
node ~/.codex/get-shit-done/bin$gsd-tools.js state update-progress

# Record execution metrics
node ~/.codex/get-shit-done/bin$gsd-tools.js state record-metric \
...
node ~/.codex/get-shit-done/bin$gsd-tools.js state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.codex/get-shit-done/bin$gsd-tools.js state add-blocker "Blocker description"
```
</step>
...
```bash
node ~/.codex/get-shit-done/bin$gsd-tools.js state record-session \
...
```bash
node ~/.codex/get-shit-done/bin$gsd-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.codex/get-shit-done/bin$gsd-tools.js commit "" --files .planning/codebase/*.md --amend
```
</step>
